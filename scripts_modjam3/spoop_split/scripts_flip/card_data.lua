...
---@param slot PillCardSlot
---@return integer
function CardjamFlipCards:getCardData(player, slot)
    local data = CardjamFlipCards:getData(player)
...
---@param value integer
---@return nil
function CardjamFlipCards:setCardData(player, slot, value)
    local data = CardjamFlipCards:getData(player)
    data.CARD_DATA = data.CARD_DATA or {0,0,0,0}
...
CardjamFlipCards:AddCallback(ModCallbacks.MC_POST_PLAYER_DROP_CARD, dropCard)
...
local function removeCard(_, player, id, slot)
    ...
    local ogVal = CardjamFlipCards:getCardData(player, slot)
    ...
    CardjamFlipCards:setCardData(player, slot, 0)
    ...
    CardjamFlipCards:setCardData(player, maxSlot+1, 0)
end
CardjamFlipCards:AddCallback(ModCallbacks.MC_POST_PLAYER_REMOVE_CARD, removeCard)
...
CardjamFlipCards:AddCallback(ModCallbacks.MC_POST_PLAYER_REMOVE_PILL, removePill)
...
local function swapPocket(_, player)
    ...
    local ogVal = CardjamFlipCards:getCardData(player, 0)
    for i=0, maxSlot-1 do
        CardjamFlipCards:setCardData(player, i, CardjamFlipCards:getCardData(player, i+1))
    end
    CardjamFlipCards:setCardData(player, maxSlot, ogVal)
end
CardjamFlipCards:AddCallback(ModCallbacks.MC_PRE_PLAYER_POCKET_ITEMS_SWAP, swapPocket)
...
local function pickUpCard(_, player, pickup)
    ...
    if(slot) then
        CardjamFlipCards:setCardData(player, slot, pickup:GetVarData())
        player:GetData().INCOMING_CARD_SLOT = nil
    end
end
CardjamFlipCards:AddCallback(ModCallbacks.MC_POST_PLAYER_COLLECT_CARD, pickUpCard)
...
CardjamFlipCards:AddCallback(ModCallbacks.MC_POST_PLAYER_ADD_CARD, addCard)
...
local function addPill(_, player, _, slot)
    ...
    CardjamFlipCards:setCardData(player, slot, -1)
    player:GetData().INCOMING_CARD_SLOT = nil
end
CardjamFlipCards:AddCallback(ModCallbacks.MC_POST_PLAYER_ADD_PILL, addPill)
...
local function addCollectible(_, id, charge, firstTime, slot, varData, player)
    if(slot==ActiveSlot.SLOT_POCKET or slot==ActiveSlot.SLOT_POCKET2) then
        ...
        if(addedSlot) then
            ...
            CardjamFlipCards:setCardData(player, addedSlot, -2)
        end
    end
end
CardjamFlipCards:AddCallback(ModCallbacks.MC_POST_ADD_COLLECTIBLE, addCollectible)
...
local function removeCollectible(_, player, id, fromPlayer, innate)
    ...
    for i=0, 3 do
        local pocketitem = player:GetPocketItem(i)
        --print("  ", i, pocketitem:GetSlot(), (pocketitem:GetSlot()>2 and player:GetActiveItem(pocketitem:GetSlot()-1) or -1), CardjamFlipCards:getCardData(i))
        if(CardjamFlipCards:getCardData(player, i)==-2 and (pocketitem:GetSlot()==0 or (pocketitem:GetType()==PocketItemType.ACTIVE_ITEM and player:GetActiveItem(pocketitem:GetSlot()-1)==0))) then
            removedSlot = i
        end
    end
    ...
    if(removedSlot) then
        ...
        CardjamFlipCards:setCardData(player, removedSlot, 0)
...
CardjamFlipCards:AddCallback(ModCallbacks.MC_POST_TRIGGER_COLLECTIBLE_REMOVED, removeCollectible)
...
local function postRender(_, pl)
    ...
    for i=0, 3 do
        Isaac.RenderText(tostring(i)..": "..tostring(CardjamFlipCards:getCardData(pl,i)), pos.X+20, pos.Y-40+i*10, 1,1,1,1)
    end
end
CardjamFlipCards:AddCallback(ModCallbacks.MC_POST_PLAYER_RENDER, postRender)
